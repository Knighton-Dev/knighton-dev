...
const navigation = {
  ...
  social: [
    {
      name: "GitHub",
      href: "https://github.com/probablynotian",
      ...
      <div className="mx-auto max-w-7xl px-6 py-12 lg:px-8">
            ...
            <ul className="flex flex-wrap gap-x-8 gap-y-2">
              {navigation.main.map((item) => (
                <li key={item.name}>
                  <Link
                    href={item.href}
                    className="text-sm text-gray-400 hover:text-[#228B22] transition-colors"
                  >
                    {item.name}
                  </Link>
                </li>
          ...
          <div className="order-2 md:order-3 flex gap-6">
            {navigation.social.map((item) => (
              <a
                key={item.name}
                href={item.href}
                className="text-gray-400 hover:text-[#228B22] transition-colors"
                target="_blank"
        ...
        <div className="mt-8 border-t border-[#262626] pt-8">
          <p className="text-center text-sm text-gray-500">
            &copy; {currentYear} Knighton Dev LLC. All rights reserved.
          </p>
        </div>
      </div>
    </footer>
